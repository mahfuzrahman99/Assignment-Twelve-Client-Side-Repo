...
const UpdateCamps = ({ showModal, setShowModal, camp }) => {
  const {
    handleSubmit,
    register,
    reset,
  ...
  const onSubmit = async (data) => {
    ...
    if (res.data.success) {
      const menuItem = {
        ...
        specialized_Services: data.specializedServices,
        healthcare_Professionals: data.healthcareProfessionals,
      ...
      const campRes = await axiosSecure.patch(`/campus/${_id}`, menuItem);
      ...
      {showModal && (
        <dialog id="my_modal_1" className="modal" open>
          <div className="modal-box max-h-[90vh] max-w-4xl mx-auto">
            <>
              <Helmet>
                <title>CareCampusPro | Update Camps</title>
              </Helmet>
              <SectionTitle
                heading={"Update campus"}
                subHeading={"Hurry Up..."}
              />
              <form
